...
$pricesell=Arrival::find()->where(['id_product'=>$res->id_product])->orderBy("id DESC")->one()->pricesell;
				...
				if (strlen($res->idProduct->name)>25) $size='10px';
				
				else $size='15px';
				$pricesell=Arrival::find()->where(['id_product'=>$res->id_product])->orderBy("id DESC")->one()->pricesell;
				
				echo "<div align='center' style='margin-bottom:14px'>".BarcodeGenerator::widget($optionsArray)."<span  style='font-size:$size'>".$res->idProduct->name."<b>$pricesell</b></span></div> </br>"; 
